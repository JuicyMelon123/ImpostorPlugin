...
- **Kick-and-spectate ruleset** — any player who dies is **kicked** (not banned) and
  told to rejoin. On rejoin, they're put into a custom "fake spectator" state
  (Adventure mode + permanent invisibility + flight + full invulnerability) so they
  can watch the rest of the round. **Not** real `GameMode.SPECTATOR` - that gamemode
  disables the hotbar entirely, which would break the item below. The trade-off is no
  true noclip through solid blocks (that's a client-only feature of real spectator
  mode); flight lets them go around/over obstacles instead. They're given a
  **nether star** in their first hotbar slot — right-clicking it opens a menu with a
  ...
  impostor immediately teleports to them and keeps following (re-teleporting once a
  second) as they move; picking crewmates opens a second menu listing every alive
  crewmate as a named player head, click one to follow them instead. Spectators can't
  take or deal damage, pick up items, lose hunger, or interact with blocks/containers.
...
- **Win conditions** — the round ends automatically and broadcasts a win message:
  ...
  - **Impostor wins** if the **round timer runs out** before the impostor is caught
- **Round timer** — a countdown shown on everyone's action bar for the whole round
  (e.g. `Kalan süre: 5:00:00`), fully configurable duration via `timer.duration-hours`
  / `-minutes` / `-seconds`. Set `timer.enabled: false` to turn it off entirely.
