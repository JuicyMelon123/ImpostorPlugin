...
public class PlayerInteractListener implements Listener {
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (gm.isAbilityDye(item)) {
            ...
            if (nowActive) {
                ...
                com.selim.impostorgame.Utils.sendActionBar(player, gm.msg("messages.ability-on"));
            } else {
                player.removePotionEffect(PotionEffectType.RESISTANCE);
                player.removePotionEffect(PotionEffectType.STRENGTH);
                com.selim.impostorgame.Utils.sendActionBar(player, gm.msg("messages.ability-off"));
            }
        }
    }

    /**
     * Fully suppresses vanilla's default handling for this click - both the block's
     * own interaction (opening a chest/door/etc.) and the item's own use behavior -
     * so our own logic below always runs the same way regardless of what block (if
     * any) the player happens to be looking at when they right-click.
     */
    private void denyDefaultBehavior(PlayerInteractEvent event) {
    ...
    public void onDrop(PlayerDropItemEvent event) {
        ...
        if (gm.isTrackerCompass(item) || gm.isAbilityDye(item) || gm.isSpectatorItem(item)) {
            event.setCancelled(true);
        }
    }
}
