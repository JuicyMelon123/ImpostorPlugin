...
public class ImpStartCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        // Now that gameRunning is true, chat/tab/nametag hiding turn back on -
        // apply immediately rather than waiting for the next join.
        gm.applyTabVisibility();
        for (Player p : online) {
            gm.applyNametagVisibility(p);
        }
        ...
        for (Player p : online) {
            ...
            // Clear any lingering fake-spectator state from a previous round, in case
            // someone reconnects mid-cleanup or an admin force-started a new round.
